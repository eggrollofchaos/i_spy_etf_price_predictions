...
class Pmdarima_Model_En_Ex:
    def __init__(self, endo, exog, endo_name, exog_name, train_size, n, periods,
                        freq, seas=None, f_seas=None, fit_seas=False, impute=False, verbose=1,
...
#                         n, periods, freq, seas, impute=1, verbose=1, extra=0,
#                         gridsearch=0):
        try:
        ...
        if estimate_diffs:
            kpss_diffs = pm.arima.ndiffs(self.endo_train, alpha=0.05, test='kpss', max_d=3)
            adf_diffs = pm.arima.ndiffs(self.endo_train, alpha=0.05, test='adf', max_d=3)
            self.n_diffs_en = max(adf_diffs, kpss_diffs)

            # kpss_diffs = pm.arima.ndiffs(self.exog_train, alpha=0.05, test='kpss', max_d=3)
            # adf_diffs = pm.arima.ndiffs(self.exog_train, alpha=0.05, test='adf', max_d=3)
    ...
    def __run_auto_pipeline(self, en_ex, show_summary=False, return_conf_int=False):
        # X_train = pd.DataFrame(train.index)
        # y_train = train.values
        # X_test = pd.DataFrame(test.index)
        # y_test = test.values
        if en_ex == 'endo':
            show_summary, pm.tsdisplay(self.endo_train, lag_max=60, title = f'{en_ex.title()} Time Series Visualization') \
    ...
    def __plot_test_predict(self, X_train, y_train, X_test, y_test, y_hat, ylabel, en_ex, auto=False):
    ...
    def run_auto_pipeline(self, en=True, ex=True, show_summary=False, visualize=True):
        if en:
            if self.verbose == 1:
            ...
            X_train, y_train, X_test, y_test, y_hat, conf_ints = \
                self.__run_auto_pipeline(en_ex='endo', show_summary=show_summary, return_conf_int=True)
            self.__get_model_scores(y_test, y_hat, y_train, en_ex='endo')
            if visualize:
                self.__plot_test_predict_conf(X_train, y_train, X_test, y_test, y_hat,
        ...
        if ex:
            ...
            X_train, y_train, X_test, y_test, y_hat, conf_ints = \
                self.__run_auto_pipeline(en_ex='exog', show_summary=show_summary, return_conf_int=True)
            self.__get_model_scores(y_test, y_hat, y_train, en_ex='exog')
            if visualize:
                self.__plot_test_predict_conf(X_train, y_train, X_test, y_test, y_hat,
